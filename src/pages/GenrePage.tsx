...
import { ArrowLeft, Music, Radio, Search, Play, MapPin, Loader2, ExternalLink } from 'lucide-react';
          ...
          <ScrollArea className="h-[calc(100dvh-380px)] md:h-[calc(100dvh-420px)]">
            <div className="space-y-1">
              {genreStations.map((station) => {
                ...
                return (
                  <div key={station.id} className={`w-full flex items-center gap-3 p-3 rounded-lg transition-all ${isActive ? 'bg-accent/10 border border-accent/30' : 'hover:bg-muted/50'}`}>
                    <button onClick={() => play(station)} className="flex items-center gap-3 flex-1 min-w-0 text-left" aria-label={`Play ${station.name}`}>
                    ...
                    <Link to={`/stations/${station.id}`} className="p-1.5 rounded hover:bg-muted/70 text-muted-foreground hover:text-foreground flex-shrink-0" aria-label={`Open ${station.name} station page`} title="Station details">
